...
#define PERIPH_RCC_OFFSET 0x00023800
#define RCC_BASE (PERIPH_BASE + PERIPH_RCC_OFFSET)
#define RCC_AHB1ENR_OFFSET 0x30

#define PERIPH_GPIOX_OFFSET 0x00020000
#define GPIOX_BASE (PERIPH_BASE + PERIPH_GPIOX_OFFSET)
...
void main(void) {
    ...
    //    RCC_AHB1ENR |= 0x1;
    (*((unsigned int *)(PERIPH_ALIAS_BASE + ((PERIPH_RCC_OFFSET + RCC_AHB1ENR_OFFSET) * 32) + (0 * 4)))) = 1;
    ...
    //    GPIOA_MODER |= 0x400;
    (*((unsigned int *)(PERIPH_ALIAS_BASE + (PERIPH_GPIOX_OFFSET * 32) + (10 * 4)))) = 1;
    ...
    while (1) {
        (*((unsigned int *)(PERIPH_ALIAS_BASE + ((PERIPH_GPIOX_OFFSET + GPIOA_ODR_OFFSET) * 32) + (5 * 4)))) = 1;
        delay(DELAY_PERIOD);
        (*((unsigned int *)(PERIPH_ALIAS_BASE + ((PERIPH_GPIOX_OFFSET + GPIOA_ODR_OFFSET) * 32) + (5 * 4)))) = 0;
        delay(DELAY_PERIOD);
    }
}
